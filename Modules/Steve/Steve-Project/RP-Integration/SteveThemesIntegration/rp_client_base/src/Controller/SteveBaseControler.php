...
abstract class SteveBaseControler extends ControllerBase
{
    ...
    public function __construct()
    {
        ...
        $_SESSION["region"] = strval(\Drupal::languageManager()->getCurrentLanguage()->getId());
        $_SESSION["defRegion"] = strval(\Drupal::languageManager()->getDefaultLanguage()->getId());
    ...
    public function buildMenu($menu_id, $modificators = [])
    {
        $menu = [];
        ...
        foreach ($tree as $key => $item) {
            ...
            if ($modificators && isset($modificators[$item->link->getPluginId()])) {
                $this->{$modificators[$item->link->getPluginId()]}($menu[$key]);
            }
    ...
    public function buildFooterMenu($taxonomy, $prefix)
    {
        $menu_links = [];

        if ($links = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->loadByProperties(['vid' => $taxonomy])) {
            foreach ($links as $link) {
                $url = NULL;
                $classes = [];

                if (!$link->get("field_{$prefix}_menu_classes")->isEmpty()) {
                    $classes = explode(' ', preg_replace('/(\s\s+|\t|\n)/', ' ', $link->{"field_{$prefix}_menu_classes"}->value));
                }

                if (!$link->get("field_{$prefix}_menu_link")->isEmpty()) {
                    $url = Url::fromUri($link->{"field_{$prefix}_menu_link"}->uri)->toString();
                }

                if ($link->{"field_{$prefix}_menu_popup"}->value && !$link->get("field_{$prefix}_menu_popup_id")->isEmpty()) {
                    $classes[] = 'popUp';
                    $url = $link->{"field_{$prefix}_menu_popup_id"}->value;
                }
                ...
                $menu_links[] = [
                    ...
                    'url' => $url,
                    'classes' => $classes,
                    'img' => !$link->get("field_{$prefix}_menu_image")->isEmpty() ? $this->getImgUrl($link->{"field_{$prefix}_menu_image"}->target_id) : NULL
                ];
            }
        }
    ...
    public function getBreadcrumbs()
    {
        ...
        switch ($routeName) {
            ...
            default:
                ...
                $pageTitle = \Drupal::service('title_resolver')->getTitle($request, $route->getRouteObject());
    ...
    public function getSport($id = NULL, $type = NULL)
    {
        ...
        if (!is_null($id) && !is_null($type)) {
            switch ($type) {
                case 'node':
                    $entity = Node::load($id);
                    $data['tid'] = $entity->{'field_' . $entity->type->target_id . '_sport'}->target_id;
                    break;
                case 'term':
                    $data['tid'] = $id;
                    break;
                case 'api':
                    $data['field_api_id'] = 'sport_' . $id;
                    break;
            }
        } else {
            if ($node = $this->getNodeByUrl(1)) {
                $data['tid'] = $entity->{'field_' . $entity->type->target_id . '_sport'}->target_id;
            } else if ($term = $this->getTaxonomyTermByUrl()) {
                $taxonomyType = $term->getVocabularyId();

                if ($taxonomyType == 'sport') {
                    $data['tid'] = !empty($term->field_sport_api_id->target_id) ? $term->field_sport_api_id->target_id : $term->id();
                } else if (!empty($term->{'field_' . $taxonomyType . '_sport'}->target_id)) {
                    $data['tid'] = $term->{'field_' . $taxonomyType . '_sport'}->target_id;
                }
            }
        }
    ...
    public function getTaxonomyTermByUrl()
    {
        ...
        if ($routeName == 'entity.taxonomy_term.canonical') {
            ...
            return $term;
        } else if ($routeName == 'rp_cms_steve_integration.live_stream_review') {
    ...
    {
        $node = \Drupal::entityTypeManager()->getStorage('node')->loadByProperties($obj);
        if ($reset != 0) {
            return $node;
        } else {
            return reset($node);
    ...
    public
    function getNodeAlias($id, $absolute = FALSE)
    {
        $options = [];
        if ($absolute) {
    ...
    public function getSchedulePlusTree($range = 0, $format = "Y-m-d", $days = 3, $round = 0, $sport = NULL, $time = NULL, $conditions = NULL, $modificators = NULL)
    {
        $nodes = $this->getSchedule($range, $days, $round, $sport, $time, $conditions, $modificators);

        if ($nodes) {
            $tree = $this->getTree($nodes, $format);
        } else {
            $tree = $this->getTournamentList($sport);
        }

        return $tree;
    }

    public function getSchedule($range, $days = 3, $round = 0, $sport = NULL, $time = NULL, $conditions = NULL, $modificators = NULL)
    {
        set_time_limit(120);
        ...
        $fromDate = strtotime("midnight", $time);
        $endDate = strtotime('+' . $days . ' day', $fromDate);
        ...
        if ($conditions) {
            foreach ($conditions as $condition) {
                $ids->condition($condition['field'], $condition['value'], $condition['operator']);
            }
        }

        $ids->sort('field_event_date', 'ASC')
        ...
        $nodelist = $this->getScheduleFormat($all_nodes, $modificators);
    ...
    public function getNodes($ids, $rest = false)
    {
        ...
        if ($_SESSION["region"] == $_SESSION["defRegion"]) {
            ...
            foreach ($ids as $id) {
                $node = Node::load($id);
                $node_trans = \Drupal::service('entity.repository')->getTranslationFromContext($node, $_SESSION["region"]);
                if (!$rest) {
    ...
    public
    function getScheduleFormat($nodeList, $modificators = NULL)
    {
        $newNodeList = [];
        foreach ($nodeList as $simpleNode) {
           ...
           // if (isset($participantPropertiesArray)) {
                ...
                $sportName = $this->getSport($nid, 'node');

                $participantsWeight = -1;

                foreach ($participantsListformat as $participantArray) {
                    if ($participantArray['weight'] > $participantsWeight) {
                        $participantsWeight = $participantArray['weight'];
                    }
                }
                ...
                $data = [
                    ...
                    'order' => $eventWeight,
                    'tournamentWeight' => $tournament->field_weight->value,
                ];

                if ($modificators) {
                    foreach ($modificators as $modificator) {
                        $this->{$modificator}($data, $simpleNode);
                    }
                }
    ...
    function getParticipant($participantsList, $participantPropertiesArray = [])
    {
        ...
        foreach ($participantsList as $participants) {
            if ($participants["target_id"]) {
                ...
                $participantsListFormat[] = [
                    ...
                    'order' => 1000,
                    'weight' => $tournamentContent->field_weight->value
                ];
            }
        }
        ...
        if ($participantPropertiesArray) {
            ...
            } else if (isset($participantPropertiesArray[0]["value"])) {
    ...
    function getStreamListFormat($eventstreamList = [])
    {
        ...
        foreach ($list as $listF) {
            ...
            $listFormat[] = [
                ...
                'disclaimer' => !$listF->get('field_stream_disclaimer')->isEmpty() ? $listF->field_stream_disclaimer->value : NULL
                /**/
    ...
    public function orderTree($tree)
    {
    ...
    public function getTournamentList($sport = NULL)
    {
        $data = [];
    ...
    function getMenuLinks()
    {
        ...
        $menu_link = \Drupal::entityTypeManager()->getStorage('menu_link_content')->loadByProperties(['menu_name' => 'main', 'weight' => '0', 'enabled' => true]);
        $nodes = [];
        $links = [];

        foreach ($menu_link as $sport) {
            $nid = $sport->description->value;
            $nodes[] = $this->getNodeByCriterio(['type' => 'sport', 'field_sport_sport' => $nid], 0);
        }
        ...
        foreach ($nodes as $node) {
            ...
            if ($term_id) {
                $term = $this->getTaxonomyByCriterio(['tid' => $term_id, 'vid' => 'sport']);
                if ($term->field_base_color->value) {
                    $color = $term->field_base_color->value;
                    $hsl = $this->hexToHsl(substr($color, 1));
                    $link['colors'] = [
                        'main' => $color,
                        'gradient_dark' => '#' . $this->hslToHex([$hsl[0], $hsl[1], $hsl[2] - 0.1 > 0 ? $hsl[2] - 0.1 : 0]),
                        'gradient_light' => '#' . $this->hslToHex([$hsl[0], $hsl[1], $hsl[2] + 0.1 < 0.99 ? $hsl[2] + 0.1 : 0.99]),
                    ];
                    if ($link['active']) {
                        $links['active'] = $color;
                    }
    ...
    function getStreamProviderFormat()
    {
        ...
        if ($term) {
            ...
            $info = [
                ...
                'Provider1Content' => 'Hos Viaplayfår du lidt af det hele. Du kan se alle deres egne programmer, 
                                        ligesom de også har et udemærket udvalg af film og serier, og så kan du se sport i stor stil. Som lidt 
                                        ekstra kan du gennem Viaplay leje nye filmudgivelser, som ikke er at finde i kataloget.',
                'Provider1Stars' => 4,

                'Provider2Heading' => 'Live streaming',
                'Provider2Content' => 'Det er her, at Viaplay udmærker sig for alvor. Viasat har blandt andet 
                                        rettighederne til at vise Champions League, Formel 1 og ikke mindst NHL og det kan alt sammen 
                                        streames på nettet. Men hos Viaplay kan du også lade dig underholde af mere nicheprægede sportsgrene 
                                        som eksempelvis dart, Nascar, atletik og speedway. Der er rigeligt at tage fat på, så det er bare at 
                                        slå sig løs.',
                'Provider2Stars' => 5,

                'Provider3Heading' => 'Support',
                'Provider3Content' => 'Man kan kontakte Viaplays kundesupport på flere måder. Enten ved at ringe til dem 
                                        på tlf: 77 331 301 i tidsrummet mellem klokken 9-20 på hverdage og 12-18 i weekenden. Eller du kan skrive 
                                        til dem på support@viaplay.dk. Der udover er der mulighed for at skrive til dem gennem deres supportforum 
                                        eller Facebook.',
                ...
                'Provider4Content' => 'Det er nemt at bruge Viaplay. Alt der delt ind i kategorier om lige at finde. 
                                       Så er det bare at dobbeltklikke på, hvad man gerne vil se, og så er man i gang. Kvaliteten er der heller
                                       ikke noget at klage over, det kører. Under kategorien sport er dagens udvalg listet op, lige til at gå til.',
                'Provider4Stars' => 4,

                'Provider5Heading' => 'Vores erfaring',
                'Provider5Content' => 'Vi har ikke haft de store problemer. Live stream kører, det er nemt at bruge. 
                                        De skifter fint i indholdet, så der altid er noget, 
                                        man kan underholde sig med. Dog kunne vi godt tænke os, at de havde en livechat. 
                                        Skal man have svar hurtigt, er det nødvendigt at ringe. 
                                        De andre muligheder kan tage noget tid før, der kommer respons.',
                'Provider5Stars' => 4,

                'Provider6Heading' => 'Konklusion',
                'Provider6Content' => 'Generelt er vi glade for Viaplay. Det er helt klart på sporten, at de er bedst, 
                                        men her har de også nogle af de helt tunge rettigheder. Eneste minus er, at man kun kan bruge abonnementet 
                                        på fem enheder, hvilket er noget, som kan give problemer. Skal man skifte enhed kræver det, at man ringer
                                        til kundeservice.',
                'Provider6Stars' => 4,
    ...
    public
    function getStreamByChannel($eventProperties = array())
    {
        $region = $_SESSION["region"];
        $streamActiveList = array();
    ...
    {
        echo "<div class='col-sm-offset-3 col-sm-6 alert alert-danger'> <h1>Result...</h1>
                <pre style='color: #31708f;background-color: #d9edf7;border-color: #bce8f1;padding: 15px'>";
    ...
    public function getColors()
    {
        ...
        if ($color) {
            ...
            $colors = [
                'main' => $color,
                'button_gradient_dark' => '#' . $this->hslToHex([$hsl[0], $hsl[1], $hsl[2] - 0.1 > 0 ? $hsl[2] - 0.1 : 0]),
                'button_gradient_light' => '#' . $this->hslToHex([$hsl[0], $hsl[1], $hsl[2] + 0.1 < 0.99 ? $hsl[2] + 0.1 : 0.99]),
