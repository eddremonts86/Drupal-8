...
class TaxonomyChannel extends taxonomy
{
  ...
  public function getAllchannels()
    {
        ...
        foreach ($AllChannel as $channel) {
            $oldChanel = $this->getTaxonomyByCriterio($channel['id'], 'field_api_id');
            if (empty($oldChanel) or !isset($oldChanel)) {
                ...
                $oldChanel->field_api_id = $channel['id'];
  ...
  public function getChannelList(){
    ...
    $result=[];
    $listChannels = $this->getTaxonomyByCriterioMultiple($obj,1);
    foreach ($listChannels  as $Channels )   {
      $result[$Channels->field_api_id->value] = $Channels->field_channel_name->value;
    }
    return $result;
  }
