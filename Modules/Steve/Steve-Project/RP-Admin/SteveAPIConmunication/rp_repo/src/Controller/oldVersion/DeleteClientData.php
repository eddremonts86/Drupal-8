...
class DeleteClientData extends ControllerBase
{
  ...
  public function deleteNodeProccess()
  {
    ...
    foreach ($ids as $nid) {
      $node = NODE::load($nid);
      print "Delete node type - (".$node->getType().")- '" . $node->getTitle() . "'";
      print "\n";
      $node->delete();
    }
    return true;
  ...
  public function deleteTaxonomyItems()
  {
    ...
    foreach ($ids as $id) {
      ...
      if (!empty($taxonomy)) {
        print "Delete Taxonomy type - (".$taxonomy->getVocabularyId().") - '" . $taxonomy->getName() . "'";
        print "\n";
        $taxonomy->delete();
      }
    }
  ...
  public function deleteNodeByType($type)
  {
    ...
    foreach ($ids as $nid) {
      $node = NODE::load($nid);
      print "Delete node type (".$type.")'" . $node->title() . "' :" . $nid;
      print "\n";
      $node->delete();
    }
    return true;
