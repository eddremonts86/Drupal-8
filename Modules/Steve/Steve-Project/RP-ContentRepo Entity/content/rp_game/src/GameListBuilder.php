    ...
    public function buildGameInfoLinks($entity)
    {
        ...
        if (!empty($games_info)) {
            foreach ($games_info as $id => $game_info) {
                $games_info_combos[$game_info->id()] = [
                    $game_info->get('field_game_info_site')->entity->id(),
                    $game_info->get('field_game_info_region')->entity->id(),
                    $game_info->get('field_game_info_language')->entity->id()
                ];
            }
        }
        ...
        foreach ($entities as $id => $entity) {
            ...
            foreach ($entity->get('field_site_languages') as $item_l) {
                if ($item_l->entity) {
                    ...
                    $languages_code[] = $item_l->entity->get('field_language_content_locale')->value;
                }
            }
            $sites[$id] = [
                'name' => $entity->getName(),
