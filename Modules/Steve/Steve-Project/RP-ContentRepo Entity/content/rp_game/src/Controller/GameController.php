...
namespace Drupal\rp_game\Controller;
 ...
 *
 * @package Drupal\rp_game\Controller
 */
class GameController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionPageTitle($game_revision) {
    $game = $this->entityManager()->getStorage('game')->loadRevision($game_revision);
    return $this->t('Revision of %title from %date', ['%title' => $game->label(), '%date' => format_date($game->getRevisionCreationTime())]);
  }

  /**
   * Generates an overview table of older revisions of a Game .
   *
   * @param \Drupal\rp_game\Entity\GameInterface $game
   *   A Game  object.
   *
   * @return array
  ...
  public function revisionOverview(GameInterface $game) {
    ...
    foreach (array_reverse($vids) as $vid) {
      /** @var \Drupal\rp_game\GameInterface $revision */
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              ...
              Url::fromRoute('entity.game.translation_revert', ['game' => $game->id(), 'game_revision' => $vid, 'langcode' => $langcode]) :
              Url::fromRoute('entity.game.revision_revert', ['game' => $game->id(), 'game_revision' => $vid]),
            ];
          }
          ...
          if ($delete_permission) {
            $links['delete'] = [
              'title' => $this->t('Delete'),
              'url' => Url::fromRoute('entity.game.revision_delete', ['game' => $game->id(), 'game_revision' => $vid]),
            ];
          }
