...
class CreateClientData extends ControllerBase
{
      ...
      if (empty($actualStream)) {
        ...
        if (empty($taxonomyStreamType)) {
          $obj = [
            ...
            'field_jsonld_struct' => ($getInfoObj->getTaxonomyByCriterioMultiple(array('vid' => 'jsonld_', 'name' => 'Streams')))->id(),
          ];
  ...
  public function createTournametTaxonomy($tournament, $voc, $sport_tags, $sport_id, $logo, $logo_modified, $locale = [])
  {
    ...
    if (isset($taxonomy) or $taxonomy == false) {
      ...
      //    var_dump($competition[$i]["locales"]);exit();
    ...
    foreach ($partArray as $team) {
      ...
      if (empty($taxonomyCompetition)) {
        ...
        $obj = [
          ...
          'field_jsonld_struct' => ($getInfoObj->getTaxonomyByCriterioMultiple(array('vid' => 'jsonld_', 'name' => 'Participants')))->id(),
        ];
        $this->createGenericTaxonomy($obj);
        $taxonomyCompetition = $getInfoObj->getTaxonomyByCriterio($id, $opc);
  ...
  public function createSportPages($sportDrupalId, $sportApiId, $region, $color = "")
  {
    ...
    if (empty($TaxonomyObj)) {
      ...
      $taxonomySportId = $this->defaultTournamentTaxonomy('', $sportName, 'sport', $sportDrupalId, 'null', $sportDrupalId, 'Sport', $color,$locales);
    ...
    if ($taxonomy == FALSE) {
      if ($voc = 'sport') {
        $term = Term::create([
          ...
          'field_locales' => $locales,
          'field_jsonld_struct' => ($getInfoObj->getTaxonomyByCriterioMultiple(array('vid' => 'jsonld_', 'name' => $jsonld)))->id(),
        ]);
      } else {
        $term = Term::create([
          'parent' => [$parentId],
  ...
  public function createJsonLdTaxonomy($voc = 'jsonld_', $data = array('Events', 'Leages', 'Participants', 'Personal', 'Place', 'Site', 'Streams', 'Sport', 'Reviews', 'Blog'))
  {
  ...
  public function createSportInternBlog($sportTags, $sportName, $name, $id, $url, $sportApiId, $type, $region)
  {
    ...
    if ($type) {
      ...
      $createStreamPages = [
        ...
        'field_jsonld_struct' => ($getInfoObj->getTaxonomyByCriterioMultiple(array('vid' => 'jsonld_', 'name' => 'Blog')))->id(),
        'body' => [
          'value' => '',
          'summary' => '',
          'format ' => 'full_html',
